...
Процедура СобратьСтандартныеПутиУстановки()
	...
	Если ЭтоWindows Тогда
		...
		Если Это64ОС Тогда
			...
			КаталогТекущегоПользователя = ПолучитьПеременнуюСреды("LOCALAPPDATA");
			ПутьУстановкиПользователя_х86  = ОбъединитьПути(КаталогТекущегоПользователя, "Programs\1cv8_x86");
			ПутьУстановкиПользователя_х64  = ОбъединитьПути(КаталогТекущегоПользователя, "Programs\1cv8_x64");

			ДобавитьКаталоГПоиска(ПутьУстановкиПользователя_х64, РазрядностьПлатформы.x64, Неопределено);	
			ДобавитьКаталоГПоиска(ПутьУстановкиПользователя_х86, РазрядностьПлатформы.x86, Неопределено);
		...
		Иначе
			...
			КаталогТекущегоПользователя = ПолучитьПеременнуюСреды("LOCALAPPDATA");
			ПутьУстановкиПользователя = ОбъединитьПути(КаталогТекущегоПользователя, "Programs\1cv8");
			
			ДобавитьКаталоГПоиска(ПутьУстановкиПользователя, РазрядностьПлатформы.x86, Неопределено);
			ДобавитьКаталоГПоиска(ПутьУстановки_x86_1Cv8, РазрядностьПлатформы.x86, Неопределено);
			ДобавитьКаталоГПоиска(ПутьУстановки_x86_1Cv82, РазрядностьПлатформы.x86, Неопределено);
